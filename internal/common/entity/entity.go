...
func (it Item) validate() (err error) {
	...
	if len(invalidFields) > 0 {
		return fmt.Errorf("item validate failed||item=%v invalid, empty fields=[%s]", strings.Join(invalidFields, ","))
	}
	return nil
}
...
func (iq ItemWithQuantity) validate() (err error) {
	//if err := util.AssertNotEmpty(it.ID, it.Name, it.PriceID); err != nil {
	//	return err
	//}
	var invalidFields []string
